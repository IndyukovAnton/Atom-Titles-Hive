...
- [ ] Обновить корневой `.gitignore` для исключения:
  - `*lint*.txt`, `*lint*.json`, `build_output.txt`, `run_log.txt`, `*.log`
  - `src-tauri/binaries/**`, `src-tauri/data/**`
  - `*.sqlite`, `*.db`
- [ ] Проверить `src-tauri/.gitignore` (добавить `binaries/`, `data/`, `*.sqlite`).

### Этап 2: Убрать уже трекаемые артефакты из индекса
- [ ] Удалить из git индекса (без удаления с диска) все артефакты (см. git status).
- [ ] Проверить, что `git status` больше не показывает артефакты как tracked changes.

### Этап 3: Pre-commit hooks
- [ ] Подключить husky + lint-staged (root).
- [ ] На pre-commit запускать:
