...
/**
 * Обёртка над Input type="date" с интеграцией react-hook-form
 * Автоматически отображает ошибки валидации
 */
export const FormDateInput = forwardRef(({ name, label, required, description, className, ...props }, ref) => {
    const { register, formState: { errors }, } = useFormContext();
    const error = errors[name];
    const errorMessage = error?.message;
    const { ref: registerRef, ...registerProps } = register(name);
    return (_jsxs("div", { className: "space-y-2", children: [label && (_jsxs(Label, { htmlFor: name, children: [label, required && _jsx("span", { className: "text-destructive ml-1", children: "*" })] })), _jsx(Input, { type: "date", id: name, ...registerProps, ...props, ref: (e) => {
                    registerRef(e);
