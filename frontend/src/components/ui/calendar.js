...
function Calendar({ className, classNames, showOutsideDays = true, ...props }) {
    return (_jsx(DayPicker, { showOutsideDays: showOutsideDays, className: cn('p-3', className), classNames: {
            ...
            button_previous: cn(buttonVariants({ variant: 'outline' }), 'h-7 w-7 p-0 bg-transparent opacity-70 hover:opacity-100'),
            button_next: cn(buttonVariants({ variant: 'outline' }), 'h-7 w-7 p-0 bg-transparent opacity-70 hover:opacity-100'),
            month_grid: 'w-full border-collapse',
            weekdays: 'flex',
            weekday: 'text-muted-foreground w-9 h-8 font-normal text-[0.75rem] flex items-center justify-center',
            ...
            selected: '[&>button]:bg-primary [&>button]:text-primary-foreground [&>button]:hover:bg-primary [&>button]:hover:text-primary-foreground [&>button]:focus:bg-primary',
            today: '[&>button]:bg-accent [&>button]:text-accent-foreground',
            outside: '[&>button]:text-muted-foreground [&>button]:opacity-50 aria-selected:[&>button]:opacity-30',
            ...
            Chevron: ({ orientation, className: cls }) => orientation === 'left' ? (_jsx(ChevronLeft, { className: cn('h-4 w-4', cls) })) : (_jsx(ChevronRight, { className: cn('h-4 w-4', cls) })),
        }, ...props }));
}
