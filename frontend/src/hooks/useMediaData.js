...
function buildQueryKey(groupId, search) {
    return [
        ...MEDIA_QUERY_KEY,
...
export function useMediaData(selectedGroupIdOrParams, searchQueryParam, filtersParam) {
    const { selectedGroupId, searchQuery, filters } = useMemo(() => {
        if (typeof selectedGroupIdOrParams === 'object' &&
    ...
    const filteredList = useMemo(() => {
        ...
        if (filters.startDate) {
            result = result.filter((item) => item.startDate && item.startDate >= filters.startDate);
        }
        if (filters.endDate) {
            result = result.filter((item) => item.endDate && item.endDate <= filters.endDate);
        }
        if (filters.genres && filters.genres.length > 0) {
            result = result.filter((item) => item.genres?.some((genre) => filters.genres.includes(genre)));
        }
    ...
    const setMediaList = useCallback((next) => {
