...
export default function RecommendationsPage() {
  ...
  const [addModalInitialData, setAddModalInitialData] = useState<Partial<MediaEntry> | undefined>(undefined);
  ...
  const [selectedGroupId, setSelectedGroupId] = useState<number | null | 'all'>('all'); // Local state, mostly unused as we navigate on select
            ...
            <div className="container mx-auto p-6 space-y-8 animate-in fade-in duration-500 max-w-7xl relative z-10 bg-background/80 backdrop-blur-sm rounded-2xl my-4 shadow-lg">
              ...
              <div className="flex flex-col gap-3">
                <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-3">
                  ...
                  <span className="bg-gradient-to-r from-amber-500 to-yellow-500 bg-clip-text text-transparent">Your Discover Feed</span>
                </h1>
              ...
              <Tabs defaultValue="top-rated" value={activeTab} onValueChange={setActiveTab} className="w-full">
...
function TopRatedSection({ onAdd }: { onAdd: (item: RecommendationItem) => void }) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border rounded-lg bg-card/50">
        ...
        <p className="text-muted-foreground">Add some titles to your library to see top rated ones here.</p>
      </div>
    );
  }
...
function GenresSection({ onAdd }: { onAdd: (item: RecommendationItem) => void }) {
   const { data, isLoading } = useQuery({
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border rounded-lg bg-card/50">
        ...
        <p className="text-muted-foreground max-w-sm mt-2">
           Add titles to your library and rate them to help us analyse your favorite genres.
        </p>
      </div>
    );
  }

  return <RecommendationsGrid items={data || []} type="external" onAdd={onAdd} />;
}

function AiAssistantSection({ onAdd }: { onAdd: (item: RecommendationItem) => void }) {
  ...
  const mutation = useMutation({
    mutationFn: (data: { prompt: string; provider: string; apiKey?: string }) => 
      recommendationsApi.getAiRecommendations(data.prompt, data.provider, data.apiKey),
    onError: () => {
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <Card className="lg:col-span-1 h-fit border-0 shadow-lg overflow-hidden">
        ...
        <CardHeader className="pb-4">
          ...
          <CardDescription>Умные рекомендации на основе ваших предпочтений</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleGenerate} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold">API Key <span className="text-muted-foreground font-normal">(опционально)</span></label>
              <Input 
                type="password" 
                placeholder="sk-..." 
                value={apiKey}
      ...
      <div className="lg:col-span-2">
        {mutation.data ? (
          ...
          <div className="h-full min-h-[400px] flex flex-col items-center justify-center border border-dashed rounded-2xl p-12 text-center text-muted-foreground bg-gradient-to-br from-indigo-500/5 to-violet-500/5">
            ...
            <h3 className="text-lg font-semibold text-foreground">Готов к работе</h3>
            <p className="max-w-md mt-2">Опишите, что вы ищете, и наш AI проанализирует тысячи тайтлов, чтобы найти идеальное совпадение.</p>
          </div>
        )}
      </div>
    </div>
...
function RecommendationsGrid({ items, onAdd }: { items: RecommendationItem[], type?: 'internal' | 'external' | 'ai', onAdd: (item: RecommendationItem) => void }) {
    return (
                    ...
                    <Card className="h-full flex flex-col hover:shadow-2xl transition-all duration-300 overflow-hidden group border-0 shadow-md bg-card/80 backdrop-blur-sm hover:-translate-y-1">
                        <div className="relative aspect-[2/3] overflow-hidden bg-muted">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end p-4">
                                <Button size="lg" className="w-full gap-2 font-semibold shadow-lg bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70" onClick={() => onAdd(item)}>
                        ...
                        <CardHeader className="p-4 pb-2 space-y-2">
                            <CardTitle className="line-clamp-1 text-base font-bold" title={item.title}>{item.title}</CardTitle>
                            <div className="flex flex-wrap gap-1.5">
                                {(item.genres || []).slice(0, 2).map((g: string, i: number) => {
                                    ...
                                    return (
                                        <Badge key={g} variant="outline" className={`text-[10px] h-5 px-2 font-medium ${colors[i % colors.length]}`}>{g}</Badge>
                                    );
                                })}
                                {(item.category) && <Badge variant="outline" className="text-[10px] h-5 px-2 bg-muted/50">{item.category}</Badge>}
                            </div>
                        </CardHeader>
