...
export function buildChildrenMap(groups) {
    const map = new Map();
...
export function collectDescendantIds(groups, rootId) {
...
export function getNextSiblingId(groups, id) {
    const target = groups.find((g) => g.id === id);
    if (!target)
        return null;
    const siblings = sortByOrder(groups.filter((g) => (g.parentId ?? null) === (target.parentId ?? null)));
...
export function resolveGroupDrop(params) {
    ...
    if (activeId === targetId)
        return null;
    if (collectDescendantIds(groups, activeId).has(targetId))
        return null;
 ...
 */
export function isNoopMove(groups, activeId, resolution) {
    const active = groups.find((g) => g.id === activeId);
    if (!active)
        return false;
    if ((active.parentId ?? null) !== resolution.parentId)
        return false;
 ...
 */
export function buildParentOptions(groups, excludeId) {
    const childrenMap = buildChildrenMap(groups);
    const excluded = excludeId !== undefined
