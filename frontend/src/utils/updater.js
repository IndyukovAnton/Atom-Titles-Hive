...
export const RELEASES_URL = 'https://github.com/IndyukovAnton/Atom-Titles-Hive/releases/latest';
export const openReleasesPage = () => {
    window.open(RELEASES_URL, '_blank', 'noopener,noreferrer');
};
/** Скачивает и ставит обновление с прогресс-тостом, затем перезапускает приложение. */
async function downloadInstallAndRelaunch(update) {
    let total = 0;
    let downloaded = 0;
 ...
 */
export async function runInteractiveUpdateCheck() {
    if (!isTauri()) {
        openReleasesPage();
        return;
    }
    ...
    try {
        ...
        toast.error('Не удалось проверить обновления. Откройте страницу релизов вручную.', {
            id: checkingToast,
